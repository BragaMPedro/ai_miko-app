...
export const ConfigModal = ({ setShowModal, setChat }: ConfigModalProps) => {
   const [mikoState, setMikoState] = useState<boolean>(false);
   const [showTab, setShowTab] = useState<1 | 2 | 3>(1);
   const apiKey = process.env.NEXT_PUBLIC_API_KEY;
   const stateColor = mikoState ? "purple-600" : "fuchsia-400";
      ...
      <dialog id="my_modal_5" className="modal modal-open modal-bottom sm:modal-middle">
         <div className="modal-box p-0 h-full space-y-6 flex flex-col items-center bg-stone-800">
            ...
            <div role="tablist" className="tabs flex flex-1 w-full">
               <div
                  ...
                  <div className="flex h-full w-full relative -z-0 overflow-clip">
                     <DynamicImageSelector
                        ...
                        title="Miko Base"
                        desciption="Conversar, ou conhecer o santuário sem preocupações externas..."
                     />
                     <DynamicImageSelector
                        onClick={() => setMikoState(state => !state)}
                        changeHandler={mikoState}
               ...
               <div
                  role="tabpanel"
                  className={`tab-content flex-1 flex flex-col items-center justify-center p-6 ${
                     showTab != 2 && "hidden"
                  }`}>
                  Soon
               </div>
               ...
               <div
                  role="tabpanel"
                  className={`tab-content flex-1 flex flex-col items-center justify-evenly rounded-box p-6 ${
                     showTab != 3 && "hidden"
                  }`}>
                  Suas Configurações
                  <h2
                     className={`text-center ${
                        mikoState ? "text-rose-200 border-purple-800 p-2" : "text-purple-600 border-slate-300 p-2"
                     }`}>
            ...
            <form method="dialog" onClick={handleClose} className="modal-backdrop">
               <button>close</button>
            </form>
         </div>
      </dialog>
